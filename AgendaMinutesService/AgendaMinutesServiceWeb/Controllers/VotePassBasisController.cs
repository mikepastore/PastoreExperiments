...
{
    public class VotePassBasisController : ApiController
    {
	    [HttpGet]
        public AMAPI.VotePassBasis Get(Int32 id, string fields=null)
        {
            var connection = new HTTPContextConnectionInfoProvider().GetConnectionInfo();
            var service = VotePassBasisService.Create(connection);
