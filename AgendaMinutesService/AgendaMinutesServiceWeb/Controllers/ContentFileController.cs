...
{
    public class ContentFileController : ApiController
    {
	    [HttpGet]
        public AMAPI.ContentFile Get(Int32 id, string fields=null)
        {
            var connection = new HTTPContextConnectionInfoProvider().GetConnectionInfo();
            var service = ContentFileService.Create(connection);
