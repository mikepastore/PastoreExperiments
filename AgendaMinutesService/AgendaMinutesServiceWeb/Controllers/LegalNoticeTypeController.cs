...
{
    public class LegalNoticeTypeController : ApiController
    {
	    [HttpGet]
        public AMAPI.LegalNoticeType Get(Int32 id, string fields=null)
        {
            var connection = new HTTPContextConnectionInfoProvider().GetConnectionInfo();
            var service = LegalNoticeTypeService.Create(connection);
