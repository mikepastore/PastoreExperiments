...
{
    public class FileFormatController : ApiController
    {
	    [HttpGet]
        public AMAPI.FileFormat Get(String id, string fields=null)
        {
            var connection = new HTTPContextConnectionInfoProvider().GetConnectionInfo();
            var service = FileFormatService.Create(connection);
