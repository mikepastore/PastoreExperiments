...
{
    public class ResolutionSaveReasonController : ApiController
    {
	    [HttpGet]
        public AMAPI.ResolutionSaveReason Get(Int32 id, string fields=null)
        {
            var connection = new HTTPContextConnectionInfoProvider().GetConnectionInfo();
            var service = ResolutionSaveReasonService.Create(connection);
