...
{
    public class SpeakerSignupController : ApiController
    {
	    [HttpGet]
        public AMAPI.SpeakerSignup Get(Int32 id, string fields=null)
        {
            var connection = new HTTPContextConnectionInfoProvider().GetConnectionInfo();
            var service = SpeakerSignupService.Create(connection);
