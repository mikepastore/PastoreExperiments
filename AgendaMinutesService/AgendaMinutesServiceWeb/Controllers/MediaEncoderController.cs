...
{
    public class MediaEncoderController : ApiController
    {
	    [HttpGet]
        public AMAPI.MediaEncoder Get(Int16 id, string fields=null)
        {
            var connection = new HTTPContextConnectionInfoProvider().GetConnectionInfo();
            var service = MediaEncoderService.Create(connection);
