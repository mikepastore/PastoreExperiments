...
{
    public class FunctionalCategoryController : ApiController
    {
	    [HttpGet]
        public AMAPI.FunctionalCategory Get(Int32 id, string fields=null)
        {
            var connection = new HTTPContextConnectionInfoProvider().GetConnectionInfo();
            var service = FunctionalCategoryService.Create(connection);
